...
* `GET /api/users/balance/withdrawals` — получение информации о выводе средств с накопительного счёта пользователем.
...
```
POST /api/users/orders HTTP/1.1
...
Хендлер: `POST /api/users/balance/withdraw`
...
```
POST /api/users/balance/withdraw HTTP/1.1
Content-Type: application/json
...
Хендлер: `GET /api/users/balance/withdrawals`.
...
```
GET /api/users/withdrawals HTTP/1.1
Content-Length: 0
```
